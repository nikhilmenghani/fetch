...
class Constants:
    ...
    def update_android_version_dependencies():
        ...
        if str(Config.RELEASE_TYPE).lower.__eq__("production"):
    ...
    def get_import_path(app_set, pkg, install_path, export_directory=None):
        ...
        dir_name = str(dir_name).replace("\\system", "").replace("/system", "")
        dir_name = str(dir_name).replace("\\product", "").replace("/product", "")
